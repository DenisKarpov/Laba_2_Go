...
type Output struct {
	...
	Response *Other  `json:"response,omitempty"`
	Data     *ListOfDoc `json:"data,omitempty"`
}
...
func main() {
	...
	router := web.New(Context{}).
		...
		Middleware((*Context).Authentication).
		Get("/auth", (*Context).LoginPage).
		Get("/docs", (*Context).Documents).
		Get("/docs/:id", (*Context).DocOunput).
		Get("/load", (*Context).DocumentsLoad).
		Get("/logout", (*Context).Logout).
		Post("/create", (*Context).DocumentsCreate).
		Post("/authform", (*Context).AuthFanc).
...
//ClearTable - чистка токенов, которые не активны более 5 минут
func ClearTable() (err error) {
...
//Authentication - check user authorization
func (c *Context) Authentication(rw web.ResponseWriter, r *web.Request, next web.NextMiddlewareFunc) {
	...
	err := db.QueryRow("select un from session_table where suuid = $1", token).Scan(&CharName)
	if err != nil {
		rw.WriteHeader(401) 
...
func (c *Context) AuthFanc(rw web.ResponseWriter, req *web.Request) {
...
//DocOunput - Output one document by id
